...
NOTE: This feature works with IBM Blue Box Local Cloud ONLY. It does not apply to the IBM Blue Box Dedicated Cloud offering. The SoftLayer Network currently requires us to use L2 POP, a driver designed to support scaling to large networks. To quote one of our engineers: "The L2Population mechanism driver we need to use to make VXLAN work on SoftLayer is incompatible with MAC learning or ARP responses used by HA IP methods." As a possible workaround, in the OpenStack Mitaka release, there is an option to disable half of the L2 POP responder, which could allow you to set up HA. IF you really need HA, it's best to use a Load Balancer for that, such as Neutron LBaaS v2. As another possible workaround, if your cloud is deployed within a single SoftLayer Pod, and it will never grow beyond that single Pod, you could possibly implement some type of HA IP in a custom way.
