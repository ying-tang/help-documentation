...
# Description: 
# This script is designed to copy image assets (only .png and .svg files) from the 'img' directory in the project root to their respective language subdirectories (e.g. 'img/es') 
...
for lang in "${langs[@]}"
do
  find . -type d \($ignored_dirs \) -prune -o \( -iname '*.svg' -o -iname '*.png' \) -print | xargs -I {} cp -n {} $lang/{}
done
